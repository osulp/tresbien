...
class ReimbursementRequestsController < ApplicationController
  ...
  def reimbursement_request_params
    params.require(:reimbursement_request).permit(
      ...
      travel_itineraries_attributes: %i[id city state break lunch dinner hotel amount date _destroy],
      travel_cities_attributes: %i[id from_date to_date city state include_meals hotel_rate _destroy]
    )
  end
